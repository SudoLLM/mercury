...
from models.task import TaskStatus, create_task, query_task, update_task
...
async def azure_tts(text: str, audio_profile: str, output_dir: str):
    # random file name for the audio file
...
class InferText2AudioPayload:
    def __init__(
        self, text: str, model_name: str, audio_profile: str, mode: AudioMode, gen_srt: bool, user_id: int
    ):
        self.text = text
...
async def infer_text2audio_task_handler(task_id: int, payload_str: str) -> None:
    ...
    if payload.gen_srt:
        srt_file = await query_file(file_id=task.res["output_srt_file_id"])
        output_srt_path = srt_file.path
...
async def infer_text2video_task_handler(task_id: int, payload_str: str) -> TaskStatus:
    ...
    output_video_path = video_file.path
...
async def infer_audio2video_task_handler(task_id: int, payload_str: str) -> TaskStatus:
    ...
    # infer video asyncously
    await internal_infer_video(get_file_absolute_path(input_audio_file.path), model, video_file.path, task_id)
    return TaskStatus.PENDING


infer_audio2video_queue = TaskQueue(
    "INFER_AUDIO2VIDEO", handler=infer_audio2video_task_handler, handle_sleep=1, max_parallel_tasks=1
)
...
async def internal_infer_video(audio_path: str, model: Model, output_video_path: str, task_id: int):
    logger.debug(
    ...
    if response.status_code != 200:
        ...
        raise Exception(
            f"internal_infer_video err, response code: {response.status_code}, response: {response}"
        )
...
async def infer_asr(body: AudioAsrRequest, req: Request):
    ...
    res = await file_model.query_file(body.file_id)
    if not res:
